const quizData = [
  ...
  { question: "ゆはいちゃんねるがデュエプレに初めて課金した理由は？", choices: ["アクアンを4枚作るため", "モテるため", "企画の罰ゲーム", "コラボのスキンのため"], answer: "アクアンを4枚作るため" },
...
function startQuiz() {
  currentQuestions = shuffleArray([...quizData]).slice(0, 7); // ←ランダム7問
...
function selectAnswer(choice) {
  ...
  resultText.textContent = isCorrect ? "✅ 正解！" : `❌ 不正解…（答え: ${currentQuestion.answer}）`;
  resultText.className = isCorrect ? "correct" : "incorrect";
  document.querySelector(".quiz-container").appendChild(resultText);
...
function showScore() {
  resetState();
  questionElement.textContent = `🎉 結果発表 🎉`;
  const scoreText = document.createElement("div");
  scoreText.textContent = `あなたのスコアは ${score} / ${currentQuestions.length} 点！`;
  scoreText.style.fontSize = "24px";
  scoreText.style.marginTop = "20px";
  choicesContainer.appendChild(scoreText);
